...
graph TB
    ...
    D --> L[CTU_UDINT - 增计数器]
    D --> M[CTD_UDINT - 减计数器]
    D --> N[CTUD_UDINT - 增减计数器]
    
    E --> O[CTU_ULINT - 增计数器]
    E --> P[CTD_ULINT - 减计数器]
    E --> Q[CTUD_ULINT - 增减计数器]
...
|---------|---------|---------|---------|---------|---------|
| CTU_DINT | 32位增计数器 | 计数2次，预设值5 | CV=2, Q=FALSE | `CTU_DINT_CountValue`, `CTU_DINT_QReached` | ✅ OK |
| CTD_DINT | 32位减计数器 | 从预设值5减计数1次 | CV=4, Q=FALSE | `CTD_DINT_CountValue`, `CTD_DINT_QReached` | ✅ OK |
| CTUD_DINT | 32位增减计数器 | 先增计数2次，再减计数1次 | CV=1, QU=FALSE, QD=FALSE | `CTUD_DINT_CountValue`, `CTUD_DINT_QU`, `CTUD_DINT_QD` | ✅ OK |
...
## 测试目的
验证PLC扩展数据类型计数器指令的功能正确性，补充图片中缺失的扩展计数器指令测试，确保在Beremiz环境中能够正常工作。
